...
template<class T>
void IspisNiza(T niz[], int vel, const char* delimiter = ", ")   //potrebna je da se moze ispisati za razlicite tipove podataka
{
    //delimiter je vrijednost koja ce se ispisivati izmedju pojedinih vrijednosti u nizu 
    for (int i = 0; i < vel; i++)
    {
        cout << niz[i] << delimiter;   //ovdje moramo voditi racuna da nam je preklopljen operator [] za klasu T
    }
    cout << endl;
}
...
{
    //bool reverse odredjuje da li ce soritranje bili uzlazno ili silazno, ako je u for < onda se sortiraju opadajucim redoslijedom
    //a ako je > onda se sortiraju rastucim redoslijedom 
    ...
    //auto se mora koristiti kada imamo ugradjeni parametar u lambdi 
    //Lamba funkcije su funkcije bez naziva - anonimne funkcije 
    {
...
public:
    ...
    static Datum GetRandomDatum()
    {
        ...
        do
        {
            int dan = rand() % 31 + 1;  //ostatak ce biti izmedju [0-30], kako 0 nije validan dan dodajemo + 1
            int mjesec = rand() % 12 + 1; //primjenjujemo istu logiku kao za dan
...
bool operator != (const Datum& d1, const Datum& d2)
{
    return!(d1 == d2); //da bi ovo funkcionisali moramo naznaciti da se d1 i d2 nece mijenjati pa oni moraju biti const
    //s obzirom da pozivamo operator == moramo osigurati i da su ti parametri const 
}
//Z1.12
bool operator >(const Datum& d1, const Datum& d2)
{
...
{
    if (obj._dan != nullptr && obj._mjesec != nullptr && obj._godina != nullptr)  //ova provjera sluzi da ne bi dobili error 'unable to read memory' 
        //sto ce se desiti ako su dan, mjesec i godina nullptr, ova provjera obezbjedjuje da ce se vrijednosti ispisati samo ako nisu anulirane 
